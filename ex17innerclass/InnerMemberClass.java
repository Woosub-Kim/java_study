...
/*
내부클래스(내부멤버클래스)
	: 클래스 안에 정의된 클래스로 static으로 선언되지 않은
	클래스를 말한다. 
	-외부클래스의 모든멤버를 사용할수있다. 
	-정적멤버를 가질수 없다. 단 상수는 가능하다. 
	-컴파일시 외부클래스명$내부클래스명.class 파일이 생성된다.
	-안드로이드와 같은 이벤트기반의 프로그래밍에서 많이사용된다. 
 */
class OuterMember {
	...
	void outerInstanceMethod() {
		...
		System.out.println("내부클래스의 상수:"+ 
			InnerMember.MAX_INT);		
		//내부클래스의 멤버변수 접근(참조변수를 통해 접근)
		System.out.println("내부의 sameVar:"+ 
			inner.sameVar);
	}
	//정적멤버메소드
...
//		System.out.println("내부의 sameVar접근:"+
//				inner.sameVar);
	}
...
public class InnerMemberClass {
	...
	public static void main(String[] args) {
		...
		/*
		외부클래스가 아닌 다른 클래스에서 내부멤버클래스의 
		객체를 생성하려면 외부클래스의 객체를 통해서만 생성이
		가능하다. 
		 */
		//InnerMember innerClass = 
		//					new InnerMember();->에러발생
		
		OuterMember outerClass = new OuterMember();
		
		/*
		내부클래스의 멤버메소드를 접근할때는 외부클래스의 
		멤버변수를 통해서 접근 가능하다. 
		 */
		 ...
		 */
		OuterMember.InnerMember in =
				outerClass.new InnerMember();		
		OuterMember.InnerMember in2 = 
				new OuterMember().new InnerMember();		
	}
}
