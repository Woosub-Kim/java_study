...
-클래스의 정의하는 형태가 메소드를 정의하는것과 비슷하다. 
...
형식]
	...
	-> 새로운 객체를 생성하는 형태이므로 문장의 끝에 ; 을 붙여
	줘야 한다. 
 */
class Person {
	String name;
...
public class AnonymousClass {
	...
	public static void main(String[] args) {
		...
		/*
		Person타입으로 Student객체를 접근하기 위해서는 반드시
		다운캐스팅 진행후 접근해야 한다. 
		 */
		...
		: Person타입의 객체변수에 Person을 상속받은 익명객체를
		생성해서 그 참조값을 할당한 형태이다.
		
		new Person(){}; => 익명 extends Person
		과 같은 형태라 생각하면 된다. 
		
		이름이 없기때문에 부모클래스의 이름을 빌려서 객체를
		생성했다고 생각하면 된다. 
		 */
		Person anonyPerson = new Person("홍두께"){			
			int newVar;
			void newMethod() {
		...
		//((Student)anonyStudent).age = 100;
		/*
		다운캐스팅은 "(자식클래스명)부모타입의 객체" 형태로
		형변환하여 하위클래스의 멤버에 접근하기 위한 수단이다.
		그러나 익명클래스는 이름이 없기때문에 다운캐스팅 자체가
		불가능하다. 
		즉 익명클래스에서는 새롭게 정의한 멤버에 접근할수 없다.
		따라서 익명클래스는 오버라이딩의 목적으로만 사용해야
		한다. 		
		*/
