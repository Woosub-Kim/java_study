...
/*
 *  JDBC 작업을 위해서는 항상 드라이버 로드, DB연결(커넥션), 
 *  자원해제와 같은 반복적인 작업들이 필요한데 이 부분을 
 *  인터페이스로 정의하여 
 *  좀 더 편리하게 작업할 수 있도록 코드를 구성한다
 */
public interface IConnect {
	//멤버상수: interface에 선언된 변수는 무조건 public static final 로 선언되어 멤버상수가 된다 인터페이스 특징
	String ORACLE_DRIVER = "oracle.jdbc.OracleDriver";
	String ORACLE_URL = "jdbc:oracle:thin://@localhost:1521:orcl";
	//멤버메소드: 자동으로 public abstract가 붙어 추상메소드로 선언된다.
